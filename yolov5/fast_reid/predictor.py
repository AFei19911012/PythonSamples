...
def setup_cfg():
    ...
    """ car """
    cfg_.merge_from_file(abspath('fast_reid/configs/Market1501/AGW_R101-ibn.yml'))
    cfg_.merge_from_list(['MODEL.WEIGHTS', abspath('fast_reid/models/market_agw_R101-ibn.pth')])
    cfg_.freeze()
    return cfg_
...
class FeatureExtraction(object):
    def __init__(self, cfg_=setup_cfg()):
        self.cfg = cfg_
...
def test_person():
    ...
    for i in range(1, 4):
        dist = cal_cosine_dist(eval(f'feat{i}'), feats)
        for value in dist:
