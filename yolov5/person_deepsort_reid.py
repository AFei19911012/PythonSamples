# -*- coding: utf-8 -*-
"""
 Created on 2021/10/17 15:20
 Filename   : person_deepsort_reid.py
 Author     : Taosy.W
...
from deep_sort.deep_sort.deep_reid import DeepReid
from get_person_feat import cosine_similarity
...
def get_person_id(features_, feats_):
    ids = []
    for feat in features_:
        similarity = cosine_similarity(feat, feats_)
        max_index = np.argmax(similarity, axis=0)
        score = np.max(similarity, axis=0)
        if score < 0.9:
            max_index = -1
...
def draw_person(img, bbox_container, ids, names):
    ...
    for i, bbox in enumerate(bbox_container):
        ...
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, [255, 0, 0], lineType=cv2.LINE_AA)
        cv2.rectangle(img, c1, c2, [0, 0, 255], cv2.FILLED, cv2.LINE_AA)
...
def main():
    ...
    deepsort = DeepReid(cfg.DEEPSORT.REID_CKPT,
    ...
    while True:
        ...
        # 和数据库里的行人做对比，得到匹配 id
        ids = get_person_id(features, feats)
        draw_person(frame, bbox_container, ids, names)
