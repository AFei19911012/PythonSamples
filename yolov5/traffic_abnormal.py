# -*- coding: utf-8 -*-
"""
 Created on 2021/11/23 0:18
 Filename   : traffic_abnormal.py
 Author     : Taosy.W
...
def detect_demo():
    ...
    """ 图像检测器 """
    yolov5_detector = YOLOv5Detector(weights='weights/yolov5s.pt', conf_thres=0.5)
    ...
    while True:
        ...
        for info in bbox_container:
            print(info)
            """ 这个是检测 car traffic light """
            if info['class'] == 'car' or info['class'] == 'traffic light':
                label = 'normal'
            else:
                label = 'abnormal'
                break
        ...
        print('---')
        """ 显示图像 """
        cv2.putText(image_result, f'fps={fps}', (10, 40), 0, 1, [0, 255, 0], thickness=1, lineType=cv2.LINE_AA)
        cv2.putText(image_result, label, (10, 80), 0, 1, [0, 255, 0], thickness=1, lineType=cv2.LINE_AA)
